...
### 2.23 站点 extractor 静默 fallback 没用户感知 = 用户拿到劣化产物却不知（scys 5125541 401 排查反思，2026-05-26）

#### 现象

阿杜问"`https://scys.com/articleDetail/xq_topic/5125541242454854` 裁出来对不对" — 我跑 audit 全 PASS（带 `.scys-pw-profile` 的 dump-clip），告诉他 OK。但阿杜在自己 chrome 上裁出来跟我的**完全不一样**：

| | 用户 chrome 裁的 | dump-clip 跑的 |
|---|---|---|
| title | "用好 AI 跟上时代"（**错** — 文章中段某节小标题）| "【转化大于流量】..."（**对**）|
| author / published | 空 / 空 | 天堂地狱 / 2025-04-22 |
| body | 857 行 + 一张 base64 内联大图 | 1231 行完整 |

排查 4 步：
1. 怀疑 chrome 加载的不是 worktree dist/ — 让阿杜看 chrome://extensions，路径对（`obsidian-clipper-cn/dist`）✗
2. 怀疑 hot-reload alarm 死了 — 让阿杜在 service worker console 跑 `chrome.alarms.getAll()` + `chrome.storage.session.get('cnHotReloadLastMarker')` + `fetch(chrome.runtime.getURL('build-marker.txt'))` — alarm 活着、marker 一致 ✗
3. 怀疑 content.js 旧（注入时 stale）— F5 后还错 ✗
4. **让阿杜在 scys tab F12 console 看 scys-extractor 抛了什么** — `POST scys.com/shengcai-web/client/homePage/topicDetail 401 (Unauthorized)` + `[Clipper:scys-extractor] [article] HTTP 401` + `Article fetch failed for xq_topic/5125541242454854` ✓

#### 根因

阿杜 chrome 的 scys 认证 cookie 失效（前端 token 还在 localStorage 让页面看着像 logged in，但 API 调用 401）。**`extractScysArticleStandalone` 在 401 时 `return null` 而不是 throw** —— content.ts:282 那块 `.catch(...) → return null` 不会触发（没 throw），但 `scysContent = null` 仍走 fallback 到默认 Defuddle —— 裁出来的 base64 内联大图是 Defuddle 对第一张 OSS 图片的默认行为。

#### 关键认知

**Extractor 静默 fallback 必须有显式 UI 提示**，否则用户看到的劣化产物跟"正常"产物视觉无差异（同样是 frontmatter + 内容 + 图，只是 title 取错位置、author 空、body 短一半），**直到对比另一份才发现**。这种"看起来正常但其实降级"的体验比"直接报错"危害更大。

#### 已落地修复（§6.25）

`content.ts` 4 个 extractor catch + scys article null-return 都改成收集到 `ContentResponse.extractorWarnings[]`，popup 拿到后顶部渲染黄色 warning banner（i18n key `extractorFailedFallback`）。下次再遇到 401 / 网络挂 / API 改 schema，用户立刻看见，不会被"假性裁出"骗。

#### 下次预防（开新 extractor 时的 checklist）

任何新 extractor function (extractFooStructuredContent) 实现时：

- ✅ throw error 路径会自动被 content.ts 的 `.catch(...) → extractorWarnings.push(...)` 收集
- ⚠️ `return null` 路径（API 401/404/schema 不匹配等"软失败"）**不会**触发 catch — 必须在 content.ts 的 URL routing 块**额外加 `if (isFooUrl(...) && !fooContent) extractorWarnings.push(...)`** 的兜底
- 验证：用 dump-clip 模拟 fail（mock fetch 返 401 / 改 URL 测 404）看 warning 是否真出现在侧栏 banner
...
### 6.25 ~~Extractor 静默 fallback warning banner — 让降级裁剪显式可见~~（**已完成 2026-05-26**，commits `f2e3e51..dd03a1d`，2 commits on `worktree-extractor-fallback-warning-banner`）

**触发** §2.23 反思 — scys 5125541 401 排查发现"静默 fallback 用户拿到劣化产物却没感知"是 4 个站点 extractor (bilibili/feishu/scys/zsxq) 共同 footgun。

**改动**：
- `src/content.ts` — 4 个 extractor `.catch(...)` 把 error msg 收集到 local `extractorWarnings: string[]`；scys article URL 命中但 `extractScysArticleStandalone` 静默 return null (401/404 path) 额外补一条；response 末尾写入 `extractorWarnings: warnings.length > 0 ? warnings : undefined`
- `src/content.ts` + `src/utils/content-extractor.ts` — 两处独立的 `ContentResponse` interface（§2.17 双 interface 陷阱）都加 `extractorWarnings?: string[]`
- `src/popup.html` + `src/side-panel.html` — `<div class="extractor-warning-banner">` (lucide alert-triangle icon + content span + 关闭按钮)
- `src/styles/popup.scss` — `.extractor-warning-banner` 黄色 left-border + 浅黄背景 + flex 布局 + 关闭按钮 hover
- `src/core/popup.ts` — `showExtractorWarnings(warnings)` / `clearExtractorWarnings()`；在 `refreshFields` 拿到 `extractedData` 后判断 `extractorWarnings` 非空就显示
- `src/_locales/en + zh_CN + 33 个其他 locale` — `extractorFailedFallback` i18n key

**Ship gate**（自动化部分）：
- ✅ T1 `npm run build:chrome` exit 0（修了 worktree defuddle 0.16 vs 主仓 0.18.1 版本漂移）
- ✅ T2 dump-clip happy path（带 `.scys-pw-profile`）→ 产物跟 baseline `_cn-test/scys-E-5125541-audit.md` byte-identical（除 `created` 当天日期）
- ✅ T3 rsync worktree dist/ → 主仓 dist/，build-marker `1779765809399 → 1779779144970` 触发 chrome alarm hot-reload
- ✅ T4 阿杜端到端验证（scys tab F5 + 裁剪 → 顶部黄色 banner 显示"scys article: extractor returned null (likely session expired)"）— 通过

**注意点 — bridge 路径不加 warning**：`content.ts:645+` window message bridge 用于 e2e 测试（dump-clip / playwright），不走 popup → 不显示 UI banner。e2e 测试代码本身会 assert markdown / title，静默 fail 也会被 catch 到，所以 bridge 保持向后兼容不加 warning collection。
