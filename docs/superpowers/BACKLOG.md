...
### 6.7 ~~scys-extractor 收尾：callout 视觉区分~~（已合并到 §6.11）
...
### 6.11 评论 / 正文 callout 类型标记保留（合并自原 §6.7，2026-05-16 重新评估）

**当前妥协**：feishu/scys CALLOUT block 渲染为普通 `<blockquote class="feishu-callout">`，turndown 后变成 `> ` 引用，**失去 Obsidian `[!tip]` / `[!warning]` / `[!quote]+` 类型标记**。

仅 scys course 评论区采用特殊路径（直接输出 HTML `<blockquote>`，避开 markdown escape）——但也没保留类型。

**根本修复路径**：让 `feishu-extractor.renderBlock` 在 CALLOUT 块上输出 `data-callout-type="tip"|"warning"|"quote"` 属性（识别飞书 callout 的 `style.emoji_id` / `background_color`）；turndown 加自定义 rule 把 `<blockquote data-callout-type="X">...` 转 `> [!X]\n> ...` 形式。

**前提**：先把 §6.10 buildVariables reserved keys 修了——dict-overlay 这条 footgun 处理掉，turndown 自定义 rule 也容易 audit。

**状态**：未启动；2-3 hr 实施 + 跑飞书 + scys 两套 e2e regression
