...
| C8 | Phase A 改动 scope：仅 e2e bridge（content.ts L864-868），主路径 frontmatter 由 template engine 渲染不受影响。不构成 double-wire 违规（见 §9.2 详述） | [[feedback_e2e_bridge_path_double_wire]] |
...
STEP 1: 写代码（worktree 内）
  ...
  │   - CbexStructuredContent + extractCbexStructuredContent 透出 end_time
  │   - content.ts e2e bridge L864-868：用 buildCbexFrontmatter 替换简版
  │     cbex fmExtra（主路径走 template engine 不动，详见 §9.2）
  │   - cbex-extractor.test.ts 扩 end_time
  │   - cbex-extractor.e2e.test.ts (522611/522884) 扩新 frontmatter 字段断言
...
- D4. Phase A 改动只在 e2e bridge L864-868（主路径 template engine 不动；见 §9.2）；git diff 体现
...
**Scope 修正**：content.ts 实际只有一处 cbex frontmatter 拼接 — **e2e bridge 路径** L864-868：
...
主路径（popup → obsidian-note-creator → template-engine）的 cbex frontmatter 由**用户配置的模板**决定（src/utils/template-compiler/variables/filters 渲染），content.ts 主路径**没有**硬编码 cbex frontmatter；L376-379 只写 `extractedContent.subject_id/status` 让 template engine 通过 `{{subject_id}}` 之类变量引用。

因此 Phase A 改动只在 e2e bridge：用 `buildCbexFrontmatter` 输出完整 cbex 字段（含价格 / 时间 / 统计 / end_time），替换原 2 行简版 fmExtra。L869-880 obsidianNote 拼接保持原架构（前 7 个通用字段不变），但 cbex 分支的 fmExtra 数组扩成 ~11 行。

注：本改动**不影响**主路径用户体验（主路径还是走 template engine），但让 e2e bridge 输出的 markdown 含完整 cbex frontmatter，方便 §4 audit 字段表的 #5-15 价格/时间/统计字段在 frontmatter 中能被 audit。此设计跟 [[feedback_e2e_bridge_path_double_wire]] 不冲突 — 该 feedback 限制「改主路径 normalize/transform 必须同步 bridge」，本任务非 normalize/transform 改动而是 frontmatter scheme 扩展，且仅 bridge-specific（主路径走 template engine 不受影响）。

**e2e baseline 更新**：522611/522884 现有 e2e test 断言 frontmatter 含 subject_id + status；改后 frontmatter 多 ~10 行（如 start_price/cap_price/deposit/bid_start/signup_end/end_time/bid_count/followers/views 等），需要扩 e2e baseline assertion 覆盖新字段。
