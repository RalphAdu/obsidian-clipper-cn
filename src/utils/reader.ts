...
export class Reader {
	...
	private static async extractContent(doc: Document): Promise<ReaderContent> {
		...
		// Bilibili: structured extraction (chapters, transcript, summary)
	...
	static async apply(doc: Document) {
		...
		try {
			...
			if (isBilibili && bilibiliVideoId) {
				...
				article.insertBefore(playerContainer, article.firstChild);
			}
	...
	static async updateReaderContent(doc: Document, content: ReaderContent): Promise<void> {
		...
		// SPA-navigation Bilibili re-init: no live <video> capture is available
		// here (we're navigating in the standalone reader, not on bilibili.com),
		// so we synthesize the embed player from the URL alone and re-wire the
		// transcript timestamps the same way apply() does.
		const isBilibili = host.includes('bilibili.com');
		if (isBilibili && isBilibiliVideoUrl(doc.URL)) {
			const urlMatch = doc.URL.match(/\/video\/(BV[\w]+|av\d+)/i);
			const bilibiliVideoId = urlMatch ? urlMatch[1] : '';
			const pageMatch = doc.URL.match(/[?&]p=(\d+)/);
			const bilibiliPage = pageMatch ? pageMatch[1] : '1';
			...
			if (bilibiliVideoId) {
				...
				const bBrowserType = await detectBrowser();
				const isSafari = ['safari', 'mobile-safari', 'ipad-os'].includes(bBrowserType);
